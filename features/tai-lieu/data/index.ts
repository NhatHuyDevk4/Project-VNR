import { PostType } from "@/common/types/post.type";

export const posts: PostType[] = [
  {
    id: 1,
    slug: "cot-moc-1-tinh-tat-yeu-thanh-lap-dang",
    title: "Cột mốc 1: Tính tất yếu thành lập Đảng Cộng sản Việt Nam",
    milestone: "1927",
    shortDescription:
      "Trong tác phẩm Đường Kách mệnh (1927), Nguyễn Ái Quốc khẳng định vai trò tiên quyết của Đảng đối với sự nghiệp giải phóng dân tộc bằng những luận điểm khoa học.",
    content:
      '## 1. Sự khủng hoảng và lời giải từ lịch sử\nĐầu thế kỷ XX, các phong trào yêu nước Việt Nam (Cần Vương, Duy Tân, Đông Du...) thất bại không phải vì thiếu nhiệt huyết, mà vì thiếu một **đường lối cứu nước đúng đắn** và một **tổ chức lãnh đạo tiên phong**.\n\n## 2. Luận điểm "Người cầm lái" trong Đường Kách mệnh\nNăm 1927, trong tác phẩm kinh điển *Đường Kách mệnh*, Nguyễn Ái Quốc đã sử dụng một hình ảnh ẩn dụ xuất sắc để giải thích vai trò của Đảng:\n\n> "Đảng có vững cách mệnh mới thành công, cũng như người cầm lái có vững thuyền mới chạy."\n\nNgười khẳng định cách mạng không phải là sự nổi dậy tự phát, mà là một khoa học. Muốn thắng lợi, phải có lý luận soi đường: *"Đảng muốn vững thì phải có chủ nghĩa làm cốt... Bây giờ học thuyết nhiều, chủ nghĩa nhiều, nhưng chủ nghĩa chân chính nhất, chắc chắn nhất, cách mệnh nhất là chủ nghĩa Lênin."*\n\n## 3. Sự sáng tạo trong quy luật thành lập Đảng\nTheo lý luận Mác - Lênin kinh điển ở châu Âu: \n$$ \\text{Đảng CS} = \\text{CN Mác - Lênin} + \\text{Phong trào công nhân} $$\n\nTuy nhiên, với nhãn quan chính trị sắc bén về một nước thuộc địa nông nghiệp, Hồ Chí Minh đã bổ sung một nhân tố thứ ba quan trọng, tạo nên tính đặc thù của Việt Nam:\n\n$$ \\text{Đảng CSVN} = \\text{CN Mác - Lênin} + \\text{Phong trào công nhân} + \\textbf{Phong trào yêu nước} $$\n\n**Ý nghĩa:** Đảng không chỉ là đội tiên phong của giai cấp công nhân mà còn là đại biểu của cả dân tộc Việt Nam.',
    image: [
      "https://tse2.mm.bing.net/th/id/OIP.mkYQr2h_MIVgkLE08HsBhwHaE_?cb=ucfimg2&ucfimg=1&rs=1&pid=ImgDetMain&o=7&rm=3",
    ],
    linkResource: [
      "https://baotanglichsu.vn/vi/Articles/1001/13639/gioi-thieu-bao-vat-quoc-gia-9-djuong-kach-menh-anh-sang-cho-con-djuong-cach-mang-viet-nam.html",
      "https://baotanghochiminh.vn/phat-hanh-tac-pham-duong-kach-menh-o-italy.htm",
    ],
  },
  {
    id: 2,
    slug: "cot-moc-2-xay-dung-dang-trong-sach-vung-manh",
    title: "Cột mốc 2: Bản chất của Đảng - Đạo đức và Văn minh",
    milestone: "1960",
    shortDescription:
      "Tổng kết 30 năm lãnh đạo, Hồ Chí Minh đúc kết bản chất của Đảng không chỉ ở quyền lực chính trị, mà còn ở tầm vóc văn hóa và đạo đức.",
    content:
      '## 1. Tuyên ngôn về tính chính danh của Đảng\nTại Lễ kỷ niệm 30 năm thành lập Đảng (1960), Chủ tịch Hồ Chí Minh đã có một đúc kết nổi tiếng, trở thành kim chỉ nam cho bản chất của Đảng cầm quyền:\n\n> "Đảng ta là đạo đức, là văn minh."\n\nĐây là một mệnh đề triết học sâu sắc:\n- **Là Đạo đức:** Đảng không có lợi ích riêng, lợi ích của Đảng nằm trong lợi ích của Nhân dân. Sự hy sinh, tận hiến là thước đo giá trị.\n- **Là Văn minh:** Đảng đại diện cho trí tuệ, cho sự tiến bộ, khoa học và lương tri của thời đại, xóa bỏ những cái cũ kỹ, lạc hậu.\n\n## 2. Năm nguyên tắc "vàng" trong sinh hoạt Đảng\nĐể Đảng luôn "sống", Người đề ra 5 nguyên tắc vận hành cốt lõi:\n\n1.  **Tập trung dân chủ:** Nguyên tắc rường cột. Dân chủ để mở rộng trí tuệ, tập trung để thống nhất hành động.\n2.  **Tập thể lãnh đạo, cá nhân phụ trách:** Khắc phục tệ sùng bái cá nhân và thói vô trách nhiệm.\n3.  **Tự phê bình và phê bình:** Đây là quy luật phát triển, là "thuốc rửa mặt hàng ngày" để ngăn ngừa suy thoái.\n4.  **Kỷ luật nghiêm minh, tự giác:** Sức mạnh không đến từ sự cưỡng bức, mà từ sự giác ngộ lý tưởng.\n5.  **Đoàn kết thống nhất:** *"Giữ gìn sự đoàn kết nhất trí của Đảng như giữ gìn con ngươi của mắt mình"*.',
    image: [
      "https://tse3.mm.bing.net/th/id/OIP.C_x9WTo0HTyIEPO3G2iYVwHaGJ?cb=ucfimg2&ucfimg=1&rs=1&pid=ImgDetMain&o=7&rm=3",
    ],
    linkResource: [
      "https://tulieuvankien.dangcongsan.vn/ban-chap-hanh-trung-uong-dang/dai-hoi-dang/lan-thu-iii",
      "https://nhandan.vn/dau-an-cac-ky-dai-hoi-dai-hoi-iii-xay-dung-cnxh-o-mien-bac-dau-tranh-giai-phong-mien-nam-post445353.html",
    ],
  },
  {
    id: 3,
    slug: "cot-moc-3-tu-tuong-ve-can-bo-va-cong-tac-can-bo",
    title: "Cột mốc 3: Cán bộ - Cái gốc của mọi công việc",
    milestone: "1947",
    shortDescription:
      "Trong tác phẩm 'Sửa đổi lối làm việc' (1947), Hồ Chí Minh đặt nền tảng cho khoa học tổ chức cán bộ với mối quan hệ biện chứng giữa Đức và Tài.",
    content:
      '## 1. Vị trí của cán bộ\nHồ Chí Minh coi cán bộ là "dây chuyền" của bộ máy, là người đem chính sách của Chính phủ, của Đoàn thể thi hành trong nhân dân. Người khẳng định dứt khoát:\n\n> "Muôn việc thành công hoặc thất bại, đều do cán bộ tốt hoặc kém."\n\n## 2. Triết lý Đức - Tài\nĐây là quan điểm xuyên suốt trong việc đánh giá con người của Hồ Chí Minh, được thể hiện qua công thức biện chứng:\n\n- **Đức (Phẩm chất):** Là cái "Gốc". Người có tài mà không có đức là người vô dụng, thậm chí nguy hiểm vì họ dùng cái tài để làm hại dân, hại nước. Biểu hiện qua: *Cần, Kiệm, Liêm, Chính, Chí công vô tư*.\n- **Tài (Năng lực):** Là cái "Dụng". Có đức mà không có tài thì làm việc gì cũng khó, không mang lại hiệu quả thực tế cho cách mạng.\n\n## 3. "Dụng nhân như dụng mộc"\nTrong nghệ thuật dùng người, Người khuyên phải biết tùy tài mà dùng, không cầu toàn trách cứ:\n\n> "Người đời ai cũng có chỗ hay chỗ dở... Dùng người như dùng gỗ. Người thợ khéo thì gỗ to, nhỏ, thẳng, cong đều tùy chỗ mà dùng được."\n\nTư tưởng này đòi hỏi người lãnh đạo phải có tâm trong sáng và con mắt tinh đời để bố trí đúng người, đúng việc.',
    image: [
      "https://tse1.mm.bing.net/th/id/OIP.tqV_f6jxmRF-yNUW9dsxVAHaFw?cb=ucfimg2&ucfimg=1&rs=1&pid=ImgDetMain&o=7&rm=3",
    ],
    linkResource: [
      "https://tuyengiao.vn/hoc-tap-va-lam-theo-loi-bac/tu-tuong-ho-chi-minh-ve-can-bo-va-cong-tac-can-bo-132338",
      "https://dangcongsan.vn/tu-tuong-van-hoa/can-bo-la-cai-goc-cua-moi-cong-viec-554476.html",
    ],
  },
  {
    id: 4,
    slug: "cot-moc-4-nha-nuoc-cua-dan-do-dan-vi-dan",
    title: "Cột mốc 4: Thiết lập Nhà nước kiểu mới",
    milestone: "1945",
    shortDescription:
      "Cuộc cách mạng về thể chế: Chuyển từ người dân mất nước (thần dân) thành người làm chủ vận mệnh quốc gia (công dân).",
    content:
      '## 1. Bước ngoặt lịch sử\nCách mạng Tháng Tám thành công không chỉ giành lại độc lập mà còn khai sinh ra một mô hình nhà nước chưa từng có trong lịch sử Việt Nam: **Việt Nam Dân chủ Cộng hòa**.\n\nĐiều 1 Hiến pháp 1946 khẳng định quyền lực tối cao thuộc về nhân dân:\n> "Tất cả quyền bính trong nước là của toàn thể nhân dân Việt Nam, không phân biệt nòi giống, gái trai, giàu nghèo, giai cấp, tôn giáo."\n\n## 2. Bản chất "Tam Dân"\n- **Của Dân:** Dân là chủ, dân là gốc. Quyền lực nhà nước không phải tự nhiên mà có, mà do nhân dân ủy thác.\n- **Do Dân:** Nhà nước do dân bầu ra, do dân ủng hộ, do dân xây dựng và đóng thuế để hoạt động.\n- **Vì Dân:** Mục tiêu tối thượng của Nhà nước không phải là cai trị, mà là phục vụ lợi ích của nhân dân.\n\n> "Việc gì có lợi cho dân, ta phải hết sức làm. Việc gì hại đến dân, ta phải hết sức tránh." (Thư gửi UBND các kỳ, tỉnh, huyện và làng - 1945)\n\n## 3. Giá trị cốt lõi\nTư tưởng này đối lập hoàn toàn với tư tưởng "quan phụ mẫu" của chế độ phong kiến. Nó đặt người dân vào vị trí **Chủ thể quyền lực**, còn cán bộ nhà nước là **Công bộc** (người phục vụ).',
    image: [
      "https://tse3.mm.bing.net/th/id/OIP.cZGSJg1HcZvevT8Vy4eJOQHaFX?cb=ucfimg2&ucfimg=1&rs=1&pid=ImgDetMain&o=7&rm=3",
      "https://upload.wikimedia.org/wikipedia/commons/4/4d/Ho_Chi_Minh_portrait.jpg",
    ],
    linkResource: [
      "https://hochiminh.vn/tu-tuong-dao-duc-ho-chi-minh/noi-dung-tu-tuong-dao-duc/tu-tuong-ho-chi-minh-ve-nha-nuoc-cua-dan-do-dan-vi-dan-26",
      "https://tuyenquang.dcs.vn/DetailView/99978/22/Tu-tuong-Ho-Chi-Minh-ve-nha-nuoc-phap-quyen-cua-dan-do-dan-vi-dan.html",
    ],
  },
  {
    id: 5,
    slug: "cot-moc-5-nha-nuoc-phap-quyen-nhan-nghia",
    title: "Cột mốc 5: Tư tưởng Pháp quyền Nhân nghĩa",
    milestone: "1946",
    shortDescription:
      "Sự kết hợp độc đáo giữa 'Thần linh pháp quyền' (thượng tôn pháp luật) và đạo lý truyền thống của dân tộc.",
    content:
      '## 1. Tinh thần thượng tôn pháp luật\nNgay từ năm 1919, trong *Yêu sách của nhân dân An Nam*, Nguyễn Ái Quốc đã đòi hỏi sự thay thế chế độ cai trị bằng sắc lệnh sang cai trị bằng pháp luật:\n\n> "Bảy xin Hiến pháp ban hành,\n> Trăm điều phải có thần linh pháp quyền."\n\nKhi trở thành Chủ tịch nước, Người trực tiếp chỉ đạo soạn thảo Hiến pháp 1946. Người hiểu rõ: Dân chủ mà không có pháp luật thì sẽ dẫn đến vô chính phủ; Pháp luật mà không có dân chủ thì là độc tài.\n\n## 2. Pháp quyền nhưng phải Nhân nghĩa\nĐiểm đặc sắc trong tư tưởng Hồ Chí Minh là sự hài hòa giữa **Pháp trị** (quản lý bằng luật) và **Đức trị** (giáo dục bằng đạo đức).\n- Pháp luật phải nghiêm minh, không có vùng cấm.\n- Nhưng pháp luật cũng phải có tính nhân văn (nhân nghĩa), mục đích là để giáo dục con người hướng thiện chứ không chỉ là trừng phạt.\n\n## 3. Ý nghĩa thời đại\nĐây là tiền đề cho việc xây dựng Nhà nước Pháp quyền XHCN Việt Nam hiện nay: Nhà nước quản lý xã hội bằng pháp luật, và pháp luật phải thể hiện ý chí, nguyện vọng của nhân dân.',
    image: [
      "https://thanhnien.mediacdn.vn/Uploaded/minhnguyet/2022_05_18/bac-ho-2-8967.jpg",
      "https://upload.wikimedia.org/wikipedia/commons/4/4d/Ho_Chi_Minh_portrait.jpg",
    ],
    linkResource: [
      "https://tuyenquang.dcs.vn/DetailView/99978/22/Tu-tuong-Ho-Chi-Minh-ve-nha-nuoc-phap-quyen-cua-dan-do-dan-vi-dan.html",
      "https://tapchilichsudang.vn/tu-tuong-ho-chi-minh-ve-xay-dung-nha-nuoc-cua-dan-do-dan-vi-dan-trong-hien-phap-nam-1946.html",
    ],
  },
  {
    id: 6,
    slug: "cot-moc-6-kiem-soat-quyen-luc-nha-nuoc",
    title: "Cột mốc 6: Kiểm soát quyền lực - Chống tha hóa",
    milestone: "1946–1969",
    shortDescription:
      "Nhận diện sớm nguy cơ tha hóa của quyền lực và thiết lập 'lồng cơ chế' để nhốt quyền lực.",
    content:
      '## 1. Cảnh báo về "Căn bệnh quyền lực"\nHồ Chí Minh là người nhìn thấy rất sớm mặt trái của quyền lực. Người cảnh báo khi có chức quyền, cán bộ rất dễ mắc bệnh "quan cách mạng", lộng quyền, tham ô.\n\n> "Cậy thế mình ở trong ban này ban nọ, rồi ngang tàng phóng túng, muốn sao được vậy, coi khinh dư luận, không nghĩ đến dân."\n\n## 2. Hệ thống kiểm soát đa chiều\nNgười không chỉ kêu gọi đạo đức mà chủ trương thiết lập các cơ chế kiểm soát cụ thể:\n\n- **Từ trên xuống:** Thanh tra, kiểm tra của Đảng và Chính phủ. Xử lý nghiêm minh bất kể chức vụ (Vụ án trần Dụ Châu năm 1950 là minh chứng thép).\n- **Từ dưới lên:** Đây là kênh quan trọng nhất - **Sự giám sát của Nhân dân**.\n> "Phải để cho dân chúng phê bình cán bộ... Nếu Chính phủ làm hại dân thì dân có quyền đuổi Chính phủ."\n- **Kiểm soát lẫn nhau:** Cơ chế Quốc hội giám sát Chính phủ (thể hiện rõ trong Hiến pháp 1946).\n\n## 3. Bài học hiện đại\nTư tưởng này chính là nền tảng cho công cuộc đấu tranh phòng chống tham nhũng và các quy định về *Lấy phiếu tín nhiệm*, *Kê khai tài sản* hiện nay.',
    image: [
      "https://llct.1cdn.vn/2024/09/06/doanthanhnien.vn-content-uploads-images-_133034638776246533_13.7.-ctich-hcm.jpg",
    ],
    linkResource: [
      "https://quochoi.vn/tulieuquochoi/anhtulieu/Pages/anhtulieu.aspx?ItemID=823",
      "https://dangcongsan.vn/tu-tuong-van-hoa/kiem-soat-quyen-luc-trong-cong-tac-can-bo-theo-tu-tuong-ho-chi-minh-565403.html",
    ],
  },
  {
    id: 7,
    slug: "cot-moc-7-phong-chong-tham-nhung-lang-phi",
    title: "Cột mốc 7: Tuyên chiến với giặc nội xâm",
    milestone: "1952",
    shortDescription:
      "Tham ô, lãng phí và bệnh quan liêu là kẻ thù của nhân dân, của bộ đội và của Chính phủ.",
    content:
      '## 1. Nhận diện kẻ thù giấu mặt\nNăm 1952, Hồ Chí Minh phát động phong trào *Chống tham ô, lãng phí, chống bệnh quan liêu*. Người gọi đây là **"Giặc nội xâm"**.\n\nTại sao gọi là giặc? Vì nó phá hoại từ bên trong. \n> "Tham ô, lãng phí và bệnh quan liêu, dù cố ý hay không, cũng là bạn đồng minh của thực dân và phong kiến... Nó làm hỏng tinh thần trong sạch và ý chí khắc khổ của cán bộ ta."\n\n## 2. Mối quan hệ nhân quả\nNgười chỉ ra nguồn gốc của tham nhũng là **Bệnh Quan liêu** (xa rời thực tế, xa rời nhân dân). Vì quan liêu nên không nắm được việc, dẫn đến cấp dưới làm bậy (Tham ô) hoặc làm ẩu (Lãng phí) mà không biết.\n\n## 3. Liệu pháp chữa trị\nĐể chống lại "giặc nội xâm", Người kê đơn thuốc gồm 2 vị:\n- **Pháp trị nghiêm khắc:** Trừng trị thẳng tay kẻ sâu mọt đục khoét của dân.\n- **Dân chủ rộng rãi:** \n> "Làm cho quần chúng khinh ghét tệ tham ô, lãng phí, quan liêu; biến hàng trăm, hàng ngàn con mắt của quần chúng thành những ngọn đèn pha soi sáng khắp mọi nơi."\n\n## 4. Tầm nhìn chiến lược\nĐây không phải là công việc nhất thời, mà là cuộc đấu tranh "khổng lồ", lâu dài giữa cái tốt và cái xấu, giữa đạo đức cách mạng và chủ nghĩa cá nhân trong mỗi con người.',
    image: [
      "https://th.bing.com/th/id/R.25e9f1ec691b6b3ec5dad6a765241f8b?rik=Vlgbx7JsVlkoBw&pid=ImgRaw&r=0",
      "https://upload.wikimedia.org/wikipedia/commons/4/4d/Ho_Chi_Minh_portrait.jpg",
    ],
    linkResource: [
      "https://www.quanlynhanuoc.vn/2024/08/01/phong-chong-tham-nhung-theo-tu-tuong-ho-chi-minh/",
      "https://tulieuvankien.dangcongsan.vn/c-mac-angghen-lenin-ho-chi-minh/ho-chi-minh/nghien-cuu-hoc-tap-tu-tuong/bac-ho-noi-ve-tham-nhung-va-chong-tham-nhung-2413",
    ],
  },
];

// Helper function để tìm post theo slug
export function getPostBySlug(slug: string): PostType | undefined {
  return posts.find((post) => post.slug === slug);
}

// Helper function để lấy tất cả posts
export function getAllPosts(): PostType[] {
  return posts;
}
